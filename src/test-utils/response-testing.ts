...
export class TestResponseValidator {
  ...
  static validateResponse<T>(response: ApiResponse<T>): ResponseValidationResult {
    ...
    result.isValidResponse = result.hasCorrectStructure && result.metadataValid && result.dataValid && result.typeMatches;
  ...
  static createTestResponses() {
    return {
      successResponse: ResponseBuilder.success(
        { id: 1, name: 'Test Item' },
        'Test successful',
        { requestId: 'test-123' }
      ),
      ...
      entityDeletedResponse: ResponseBuilder.entityDeleted(
        'User',
        'user-123',
        { requestId: 'test-delete' }
      ),
      
      listResponse: ResponseBuilder.list(
        [{ id: 1 }, { id: 2 }],
        10,
        'Items retrieved',
        { requestId: 'test-list' }
      ...
      searchResponse: ResponseBuilder.search(
        [{ id: 1, name: 'Match 1' }],
        'test query',
        5,
      ...
      validationResponse: ResponseBuilder.validation(
        false,
        ['Field is required'],
  ...
  static testResponseUtilities() {
    ...
    return {
      ...
      metadataHandling: {
        withAdditionalMetadata: ResponseBuilder.withMetadata(
          testResponses.successResponse,
          { customField: 'customValue', userId: 'user-123' }
        )
      }
    };
  }
  ...
  static async testResponseChaining() {
    ...
    const chainWithErrorResult = await ResponseBuilder.chain(
      operationsWithError, 
      { stopOnError: false, requestId: 'chain-error-test' }
    );

    const chainWithStopOnErrorResult = await ResponseBuilder.chain(
      operationsWithError,
      { stopOnError: true, requestId: 'chain-stop-test' }
    );
  ...
  static performResponsePerformanceTest() {
    ...
    const validationResults = responses.map(response => 
      TestResponseValidator.validateResponse(response)
    );
