...
export class TestDataFactory {
  ...
  static generateRandomData() {
    const randomString = (length: number = DEFAULT_STRING_LENGTH) => 
      Math.random().toString(UUID_RADIX + STRING_RADIX_OFFSET).substring(2, length + 2);
    ...
    return {
      ...
      date: () => new Date(Date.now() - randomNumber(0, DAYS_IN_YEAR) * HOURS_IN_DAY * MINUTES_IN_HOUR * SECONDS_IN_MINUTE * MILLISECONDS_IN_SECOND).toISOString()
    };
  }
} 