...
- Priority levels: `whisper` → `gentle_nudge` → `urgent_pulse` → `burning_focus`
- Durations: `momentary_focus` → `daily_rhythm` → `weekly_arc` → `eternal_truth`
- Status tracking: `pulsing_active` → `fulfilled_completion` → `gentle_pause` → `conscious_release`
